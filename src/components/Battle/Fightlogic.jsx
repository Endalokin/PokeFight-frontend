...
const BattleComponent = ({ computerStats, playerStats }) => {
  ...
  const startBattle = () => {
    ...
    setAttacker((prevAttacker) => ({ ...prevAttacker, HP: Math.max(0, prevAttacker.HP - damageDealtByDefender) }));
    setDefender((prevDefender) => ({ ...prevDefender, HP: Math.max(0, prevDefender.HP - damageDealtByAttacker) }));
